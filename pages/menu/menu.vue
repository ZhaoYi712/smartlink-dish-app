	...
	<view class="container">
		<view class="main" v-if="goods">
			<view class="nav">
				<view class="header">
					...
					<view class="right">
						<view class="dinein" :class="{ active: takeType != '外卖配送' }" @tap="takein"><text>到店自取</text>
						</view>
						<view class="takeout" :class="{ active: takeType == '外卖配送' }" @tap="takeout"><text>外卖配送</text>
						</view>
					</view>
				</view>
				<view class="coupon">
			...
			<view class="content">
				...
				<scroll-view class="menus" :scroll-into-view="menuScrollIntoView" scroll-with-animation scroll-y>
					<view class="wrapper">
						<view v-for="(item, index) in goods" class="menu"
							:class="{ current: item.goodsCategoryName === currentCategoryName }"
							:id="`menu-${item.goodsCategoryName}`" :key="index" @tap="handleMenuTap(item.goodsCategoryName)">
							<text>{{ item.goodsCategoryName }}</text>
							<view class="dot" v-show="menuCartNum(item.goodsCategoryName)">
								{{ menuCartNum(item.goodsCategoryName) }}
							</view>
						</view>
					</view>
				</scroll-view>
						...
						<view class="list">
							...
							<view id="categoryParent" v-for="(category, index) in goods" :key="index">
								<view class="category" id="categoryMenu">
									...
									<view class="items">
										<!-- 商品列表 begin -->
										<view v-if="category.goodsCategoryShow" class="good" v-for="(goods, key) in category.goodsList"
											:key="key">
											<image :src="goods.image" class="image" @tap="showGoodDetailModal(goods)">
											</image>
											<view class="right">
												<text class="name">{{ goods.name }}</text>
												<text
													class="tips">{{ goods.description && goods.description != '' ? goods.description : goods.name }}</text>
												<view class="price_and_action">
													...
													<view v-if="goods.isSell">
														...
														<view class="btn-group" v-else>
															<button type="default" v-show="goodsCartNum(goods.id)" plain class="btn reduce_btn"
													...
													<view v-else>
														<view class="btn-group">
															<button type="primary" disabled class="btn property_btn"
																style="text-decoration: line-through;" size="mini">
																已售空
															</button>
														</view>
													</view>
			...
			<view class="cart-box" v-if="cart && cart.length">
				...
				<view
					v-else-if="new Date().getHours() >= appProperties.businessStartTime && new Date().getHours() < appProperties.businessEndTime">
				...
				<view class="wrapper">
					...
					<view class="properties" v-if="currentGoods.goodsPropertyVos && currentGoods.goodsPropertyVos.length">
						<view class="property" v-for="(goodsPropertyVo, index) in currentGoods.goodsPropertyVos" :key="index">
							...
							<view class="values">
								<view class="value" v-for="(property, index2) in goodsPropertyVo.propertyList" :key="index2"
									:class="{ default: property.isDefault }" @tap="changePropertyDefault(index, index2)">
		...
		<popup-layer direction="top" :show-pop="cartPopupVisible" class="cart-popup">
			<template slot="content">
				<view class="top" style="background-color: #e3e3e3;">
					<uni-icons type="trash" size="15"></uni-icons>
					<text @tap="handleCartClear" style="color: #858585;">清空</text>
				</view>
...
<script>
	...
	export default {
		...
		computed: {
			...
			// 是否达到起送价
			disabledPay() {
				return this.$store.getters.getTakeType == '外卖配送' && this.getCartGoodsPrice < this.appProperties
		...
		methods: {
			...
			handleMenuTap(goodsCategoryName) {
				...
				this.$nextTick(() => (this.categoryScrollTop = this.goods.find(item => item.goodsCategoryName ==
					goodsCategoryName).top));
			},
